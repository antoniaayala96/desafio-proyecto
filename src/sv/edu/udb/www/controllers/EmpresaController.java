 ...
 * @author anton
 */
@WebServlet(name = "EmpresaController", urlPatterns = { "/empresa.do" })
public class EmpresaController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter out = response.getWriter()) {
			if (request.getParameter("op") == "" || request.getParameter("op") == null) {
				request.getRequestDispatcher("/empresa/indexEmpresa.jsp").forward(request, response);
				return;
			}
			...
			switch (operacion) {
			case "eliminard":
				eliminarDependiente(request, response);
				break;
			case "modificaro":
				modificarOferta(request, response);
				break;
			case "modificard":
				modificarDependiente(request, response);
				break;
			case "obtenerpe":
				obtenerOfertaParaEditar(request, response);
				break;
			case "recuperar":
			...
			case "nuevao":
				...
				break;
			case "nuevod":
				request.getRequestDispatcher("/empresa/nuevoDependiente.jsp").forward(request, response);
				break;
			case "insertaro":
				insertarOferta(request, response);
				break;
			case "insertard":
	...
	private void recuperarPassword(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (listaErrores.size() > 0) {
				...
				response.sendRedirect(request.getContextPath() + "/passEmpresa.jsp");
			} else {
				if (modelo.recuperarPassword(correo) > 0) {
					...
					response.sendRedirect(request.getContextPath() + "/loginEmpresa.jsp");
				} else {
					request.getSession().setAttribute("fracaso", "El proceso de recuperación no puedo terminarse. " + " Hubo un error");
					response.sendRedirect(request.getContextPath() + "/passEmpresa.jsp");
				}
	...
	private void eliminarDependiente(HttpServletRequest request, HttpServletResponse response) {
		try {
			String codigo = request.getParameter("id");
			HttpSession sesion = request.getSession();
			String codigoEmpresa = (String) sesion.getAttribute("codigoEmpresa");
			if (modelo.eliminarDependiente(codigo) > 0) {
				request.setAttribute("exito", "Dependiente eliminado exitosamente");

			} else {
				request.setAttribute("fracaso", "No se puede eliminar este dependiente");
			}
			request.getRequestDispatcher("/empresa.do?op=listard&id="+codigoEmpresa).forward(request, response);
	...
	private void validarLogin(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		try {
			...
			miEmpresa = modelo.validarLogin(correo, password);
			
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Date date = new Date();
			String fechaActual = sdf.format(date);
			modelo.actualizarVencimientos(fechaActual);
			
			
			if(miEmpresa != null) {
				String codigo = miEmpresa.getCodigo(); 
				String corte = codigo.substring(0, 3);
				request.setAttribute("empEmpresa", corte);
				request.setAttribute("codigoEmpresa", miEmpresa.getCodigo());
				request.setAttribute("passwordEmpresa", miEmpresa.getPassword());
				request.setAttribute("nombreEmpresa", miEmpresa.getNombre());
				request.getRequestDispatcher("/loginEmpresa.jsp").forward(request, response);
			}else {
	...
	private void insertarDependiente(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (listaErrores.size() > 0) {
				...
				request.getRequestDispatcher("empresa.do?op=nuevod").forward(request, response);
			} else {
				if (modelo.insertarDependiente(miDependiente, codigo) > 0) {
					request.getSession().setAttribute("exito", "Dependiente registrado exitosamente");
					response.sendRedirect(request.getContextPath() + "/empresa.do?op=listard&id="+codigo);
				} else {
					request.getSession().setAttribute("fracaso", "El dependiente no ha sido registrado." + " Ya hay un dependiente registrado con ese correo electrónico");
					response.sendRedirect(request.getContextPath() + "/empresa.do?op=listard&id="+codigo);
				}
			}
	...
	private void insertarOferta(HttpServletRequest request, HttpServletResponse response) throws ParseException, SQLException{
		try {
			...
			if (Validaciones.isEmpty(miOferta.getTitulo())) {
				listaErrores.add("El título de la oferta es obligatorio.");
			}
			if (Validaciones.isEmpty(miOferta.getDescripcion())) {
				listaErrores.add("La descripción de la oferta es obligatoria.");
			}
			...
			if (Validaciones.isEmpty(miOferta.getFechaFinalCupon())) {
				listaErrores.add("La fecha de vencimiento del cupón es obligatoria.");
			}
			if (listaErrores.size() > 0) {
				request.setAttribute("listaErrores", listaErrores);
	...
	private void modificarPassword(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (listaErrores.size() > 0) {
				request.setAttribute("listaErrores", listaErrores);
				request.getRequestDispatcher("/empresa/cambiarPassword.jsp").forward(request, response);
			} else {
		...
		try {
			...
			if (listaErrores.size() > 0) {
				...
				if (modelo.modificarDependiente(miDependiente) > 0) {
					...
					request.getSession().setAttribute("fracaso", "El dependiente no ha sido actualizado. " + "Ya hay un dependiente registrado con ese correo electrónico");
					response.sendRedirect(request.getContextPath() + "/empresa.do?op=listard&id="+codigoEmpresa);
				}
			}
		} catch (IOException | SQLException | ServletException ex) {
